...
def init_parse_argument():
    ...
    requiredNamed.add_argument('-options', required=False, dest='options', help='Additional options to select depending on the needs to improve performance and level of detail required (for example : imaging_delta, highlight_delta, violations, modules, ...')
    ...
    requiredNamed.add_argument('-modulefilter', required=False, dest='modulefilter', help='Filter by modules')
...
def generate_output_file(outputextension, header, outputdata, filepath):
    logutils = LogUtils()
    if outputdata != None:
        ...
        if outputextension == 'csv':
            ...
            try: 
                ...
                logutils.logerror('csv.Error: unexpected end of data ' + tb,True)
        ...
        logutils.loginfo('File ' + filepath + ' generated',True)
...
def add_violations_for_automated_ap(qrid, set_qr_already_processed, dic_violations_by_qualityrule, i_violations_processed, automatedactionplan_maxnumber, list_violations_to_add_to_ap):
    ...
    if qrid in set_qr_already_processed:
        logutils.loginfo("  Quality ryle already %s, skipping it " % str(qrid), False)
        return
        ...
        for componentid in dic_violations_by_qualityrule[qrid]:
            if i_violations_processed < automatedactionplan_maxnumber:
                ...
                if not cur_violation.violation.hasActionPlan:  
                    ...
                    logutils.loginfo("    +" + str(i_violations_processed) + " Adding violation to automated ap %s|%s (%s|%s pri=%s existing ap=%s) " % (str(qrid),str(componentid), str(cur_violation.violation.qrname), str(cur_violation.violation.componentNameLocation), str(cur_violation.pri_selected_bc), str(cur_violation.violation.hasActionPlan)), True)
                else:
                    logutils.loginfo("    +" + str(i_violations_processed) + " Not adding violation to automated ap, already in action plan ap %s|%s (%s|%s pri=%s existing ap=%s) " % (str(qrid),str(componentid), str(cur_violation.violation.qrname), str(cur_violation.violation.componentNameLocation), str(cur_violation.pri_selected_bc), str(cur_violation.violation.hasActionPlan)), True) 
...
if __name__ == '__main__':
    ...
    load_modules = False
    if args.options != None and "modules" in args.options:
        load_modules = True    
    ...
    try:
        ...
        try:
            ...
            logutils.loginfo(pluginfile,True)
        ...
        if m0:
            ...
            logutils.logerror('Malformed rootedurl value, abording : ' + rootedurl)
            sys.exit(-1)  
        ...
        # log params
        logutils.loginfo('********************************************')
        logutils.loginfo('script_version='+script_version,True)
        logutils.loginfo('python version='+sys.version)
        logutils.loginfo('****************** params ******************')
        logutils.loginfo('restapiurl='+restapiurl)
        logutils.loginfo('edurl='+edurl)
        logutils.loginfo('rootedurl='+rootedurl)
        logutils.loginfo('host='+host)
        logutils.loginfo('protocol='+protocol)
        logutils.loginfo('warname='+str(warname))
        logutils.loginfo('user='+str(user))
        if password == None or password == "N/A":
            logutils.loginfo('password=' + password)
        else: 
            logutils.loginfo('password=*******')
        if apikey == None or apikey== "N/A":
            logutils.loginfo('apikey='+str(apikey))
        else:
            logutils.loginfo('pwd=*******') 
        logutils.loginfo('log file='+log,True)
        logutils.loginfo('log level='+loglevel)
        logutils.loginfo('applicationfilter='+str(applicationfilter))
        logutils.loginfo('qridfilter='+str(qridfilter))
        logutils.loginfo('qrnamefilter='+str(qrnamefilter))
        logutils.loginfo('componentnamelocationfilter='+str(componentnamelocationfilter))
        logutils.loginfo('actionplanfilter='+str(actionplanfilter))
        logutils.loginfo('exclusionrequestfilter='+str(exclusionrequestfilter))
        logutils.loginfo('criticalrulesonlyfilter='+str(criticalrulesonlyfilter))
        logutils.loginfo('violationstatusfilter='+str(violationstatusfilter))
        logutils.loginfo('priminvaluefilter='+str(priminvaluefilter))
        logutils.loginfo('businesscriterionfilter='+str(businesscriterionfilter))
        logutils.loginfo('modulefilter='+str(modulefilter))
        logutils.loginfo('componentsfilter='+str(componentsfilter))
        logutils.loginfo('violationsfilter='+str(violationsfilter))
        logutils.loginfo('createexclusions='+str(createexclusions))
        logutils.loginfo('createactionplans='+str(createactionplans))
        logutils.loginfo('actionplaninputtag='+str(actionplaninputtag))
        logutils.loginfo('comment='+str(comment))
        logutils.loginfo('technofilter='+str(technofilter))
        logutils.loginfo('detaillevel='+str(detaillevel))
        logutils.loginfo('nbrows='+str(nbrows))
        logutils.loginfo('outputextension='+str(outputextension))
        logutils.loginfo('displaysource='+str(displaysource))
        logutils.loginfo('output folder='+str(outputfolder))
        
        logutils.loginfo('automatedactionplan_create='+str(automatedactionplan_create))
        logutils.loginfo('automatedactionplan_maxnumber='+str(automatedactionplan_maxnumber))
        
        
        logutils.loginfo('bload_all_data='+str(bload_all_data))
        logutils.loginfo('bload_objectviolation_metrics='+str(bload_objectviolation_metrics))
        logutils.loginfo('bload_rule_pattern='+str(bload_rule_pattern))
        logutils.loginfo('bload_objectviolation_findings='+str(bload_objectviolation_findings))
        logutils.loginfo('bload_qr_results='+str(bload_qr_results))
        logutils.loginfo('bload_bc_tch_mapping='+str( bload_bc_tch_mapping))
        logutils.loginfo('bload_components_pri='+str(bload_components_pri))
        logutils.loginfo('bload_serverdetail='+str(bload_serverdetail))
        logutils.loginfo('bload_quality_model='+str(bload_quality_model))
        ...
        logutils.loginfo('log folder='+str(logfolder))
        ...
        logutils.loginfo('********************************************') 
        rest_utils = RestUtils(restapiurl, RestUtils.CLIENT_REQUESTS, user, password, apikey)
        rest_utils.open_session()
        rest_service_aip = AIPRestAPI(rest_utils) 
    
        logutils.loginfo('Initialization',True)
        # few checks on the server
        server = rest_service_aip.get_server()
        if server != None: logutils.loginfo('server version=%s, memory (free)=%s' % (str(server.version), str(server.freememory)))
        ...
        for domain in listdomains:
            idomain += 1
            logutils.loginfo("Domain " + domain.name + " | progress:" + str(idomain) + "/" + str(len(listdomains)), True)
            # only engineering domains
            if domain.name != 'AAD':  #and domain.name == 'AED_AYYILDIZ':
                ...
                for objapp in listapplications:
                    if applicationfilter != None and not re.match(applicationfilter, objapp.name):
                        logutils.loginfo('Skipping application : ' + objapp.name)
                        continue                
                    elif applicationfilter == None or re.match(applicationfilter, objapp.name):
                        logutils.loginfo("Processing application " + objapp.name,True)
                        csvdatas = [] 
                        if outputextension != None and outputextension:
                            # testing if output file can be written
                            fpath = get_filepath(outputfolder, objapp.name, outputextension)
                            filelocked = False
                            icount = 0
                            while icount < 10 and FileUtils.is_file_locked(fpath):
                                icount += 1
                                filelocked = True
                                logutils.logwarning('File %s is locked. Please unlock it ! Waiting 5 seconds before retrying (try %s/10) ' % (fpath, str(icount)),True)
                                time.sleep(5)
                            if FileUtils.is_file_locked(fpath):
                                logutils.logerror('File %s is locked, aborting for application %s' % (fpath,objapp.name),True)
                                continue
                        # snapshot list
                        snapshots = rest_service_aip.get_application_snapshots(domain.name, objapp.id)
                        for snapshot in snapshots:
                            csvdata = []
                            outputdata = ''
                            logutils.loginfo("    Snapshot " + snapshot.href + '#' + snapshot.snapshotid)
                            ###################################################################
                            # Number of violations / snapshots
                            logutils.loginfo('Initialization (step 1/6) - Number of violations',True)
                            json_tot_violations = rest_service_aip.get_total_number_violations_json(domain.name, objapp.id, snapshot.snapshotid)
                            intotalviol = -1
                            intotalcritviol = -1
                            ...
                            logutils.loginfo('Initialization (step 2/6) - Quality model',True)
                            if not bload_quality_model:
                                logutils.loginfo("NOT Extracting the snapshot quality model")                                           
                            else:
                                json_snapshot_quality_model = rest_service_aip.get_snapshot_tqi_quality_model_json(domain.name, snapshot.snapshotid)
                            if json_snapshot_quality_model != None:
                                for qmitem in json_snapshot_quality_model:
                                    ...
                                    try:
                                        ...
                                        logutils.logwarning('Not able to get the critical contribution for %s' % str(qmitem),True)
                                    ...
                                    for tccont in qmitem['compoundedWeightTerms']:
                                        ...
                                        try:
                                            ...
                                            logutils.logwarning('Not able to load technical criterion for quality rule %s' % str(qrid),True)
                                        
                                json_snapshot_quality_model = None
                            
                            ###################################################################    
                            logutils.loginfo('Initialization (step 3/6) - Technical criteria contributions',True)
                            mapbctc = None                                     
                            if not bload_bc_tch_mapping:
                                logutils.loginfo("NOT Extracting the snapshot business criteria => technical criteria mapping")                                          
                            else:    
                                mapbctc = rest_service_aip.initialize_bc_tch_mapping(domain.name, objapp.id, snapshot.snapshotid, bcids)
                            
                            ###################################################################
                            # Components PRI
                            logutils.loginfo('Initialization (step 4/6) - Components PRI',True)
                            comppri = None
                            transactionlist = {}
                            
                            if not bload_components_pri:
                                logutils.loginfo("NOT extracting the components PRI for the business criteria")
                            else: 
                                if 1 == 1:
                                    try : 
                                        comppri = rest_service_aip.initialize_components_pri(domain.name, objapp.id, snapshot.snapshotid, bcids,nbrows)
                                    except ValueError:
                                        None
                                if detaillevel == 'Intermediate' or detaillevel == 'Full':
                                    transactionlist = rest_service_aip.init_transactions(domain.name, objapp.id, snapshot.snapshotid, criticalrulesonlyfilter, violationstatusfilter, technofilter,nbrows)
                            
                            ###################################################################
                            logutils.loginfo('Initialization (step 5/6) - Distributions',True)
                            dictdistributions = {}
                            if not bload_distributions:
                                logutils.loginfo("NOT extracting the distributions")
                            else: 
                                if detaillevel == 'Intermediate' or detaillevel == 'Full':
                                    try : 
                                        dictdistributions = rest_service_aip.get_distributions_details(domain.name, objapp.id, snapshot.snapshotid, nbrows)
                                    except:
                                        logutils.logwarning('Error extracting the distributions', True)
                            ###################################################################
                            ...
                            logutils.loginfo('Initialization (step 6/6) - Quality metrics results',True)
                            # quality rules details (nb violations, % compliance)
                            json_qr_results = None
                            
                            if generate_ac_model:
                                acmodel = ArchitectureModel(objapp.name + ' quality-rules',objapp.name + ' quality-rules.CASTArchitect', ArchitectureModel.TYPE_FORBIDDEN, 'JEE,SQL,HTML5,Cobol')
                                ac_model_content = {}
                            strmodules_filter = None
                            if load_modules: 
                                strmodules_filter = "$all"                                    
                            
                            dictmetrics, dicttechnicalcriteria, listbusinesscriteria, dictmodules = {},{},[],{}
                            if not bload_qr_results:
                                logutils.loginfo("NOT Extracting the quality metrics results")                                          
                            else:    
                                
                                #def get_qualitymetrics_results_json(self, domainname, objapp.id, snapshotfilter, snapshotids, criticalonly,  modules=None, nbrows=10000000):
                                #json_qr_results = rest_service_aip.get_qualitymetrics_results_json(domain.name, objapp.id, '-1', None, criticalrulesonlyfilter, strmodules_filter, nbrows)
                                dictmetrics, dicttechnicalcriteria, listbusinesscriteria, dictmodules = rest_service_aip.get_qualitymetrics_results(domain.name, objapp.id, snapshot.snapshotid, tqiqm, criticalrulesonlyfilter, strmodules_filter, qridfilter, nbrows)
                            for metricid in dictmetrics:
                                metric = dictmetrics[metricid]
                                if metric.type == 'quality-rules' and tqiqm.get(metricid):
                                    tqiqm.get(metricid).update({"failedchecks":metric.failedchecks,"successfulChecks":metric.successfulchecks,"totalChecks":metric.totalchecks,"ratio":metric.ratio,"addedViolations":metric.addedviolations,"removedViolations":metric.removedviolations})

                            listtccontributions = []
                            logger.info('Loading the technical criteria contributors at application level')
                            ...
                            for item_tc in dicttechnicalcriteria:
                                ...
                                for item in rest_service_aip.get_metric_contributions(domain.name, tciterator.id, snapshot.snapshotid):
                                    listtccontributions.append(item)                            

                            logutils.loginfo('Extracting violations',True)
                            logutils.loginfo('Loading violations & components data from the REST API',True)
                            list_outputviolations = []
                            dic_violations_by_qualityrule = {}

                            violationfilter = ViolationFilter(criticalrulesonlyfilter, businesscriterionfilter,technicalcriterionfilter, technofilter, None, qridfilter, qrnamefilter, modulefilter, nbrows)
                            violations = rest_service_aip.get_snapshot_violations(domain.name, objapp.id, snapshot.snapshotid, None, snapshot.href, tqiqm, listtccontributions, violationfilter)
                            #json_violations = rest_service_aip.get_snapshot_violations_json(domain.name, objapp.id, snapshot.snapshotid, violationfilter)
                            header = ''
                            if violations != None:
                                ...
                                outputline += ';Module'
                                outputline += ';Technical criteria;Business Criteria;Robustness;Efficiency;Security;Changeability;Transferability;Quality standards'
                                
                                outputline += ';PRI for selected Business criterion;PRI Security;PRI Efficiency;PRI Robustness;PRI Transferability;PRI Changeability'
                                ...
                                logutils.loginfo("Process violations to complete the output", True)
                                for objviol in violations:
                                #for violation in json_violations:
                                    i_progress_step = 25
                                    iCouterRestAPIViolations += 1
                                    currentviolurl = ''
                                    violations_size = len(violations)
                                    ...
                                    if componentsfilter != None and objviol.componenthref not in componentsfilter:
                                    ...
                                    objviol.violationid = qrrulepatternhref+'#'+objviol.componenthref 
                                    ...
                                    try:
                                        ...
                                        logutils.logdebug("processing component short name = " + str(objviol.componentShortName))
                                    except KeyError:
                                    ...
                                    if componentstatusfilter != None and objviol.componentStatus != None and componentstatusfilter != objviol.componentStatus:
                                        continue
                                    """
                                    try:
                                    ...
                                    if priminvaluefilter != None:
                                        if objviol.propagationRiskIndex and objviol.propagationRiskIndex < priminvaluefilter:
                                            continue
                                    ...
                                    if not bload_objectviolation_metrics:
                                        logutils.logdebug("NOT Extracting the component metrics") 
                                    else:
                                        if detaillevel == 'Full':
                                            #Looking in the cache first
                                            objViolationMetric = dicObjectViolationMetrics.get(objviol.componenthref)
                                            #If cache is empty we load from the rest api 
                                            if objViolationMetric == None:
                                                objViolationMetric = rest_service_aip.get_objectviolation_metrics(objviol.componenthref)
                                                if objViolationMetric != None:
                                                    dicObjectViolationMetrics.update({objviol.componenthref:objViolationMetric})
                                            else:
                                                None
                                    #####################################################################################################
                                    ...
                                    if not bload_rule_pattern:
                                        logutils.logdebug("NOT Extracting the rule pattern details")                                                
                                    else:
                                        if detaillevel == 'Full':
                                            #Looking in the cache first
                                    ...
                                    if not bload_objectviolation_findings:
                                        logutils.loginfo("NOT Extracting the component findings")
                                    ...
                                    # for rules like Avoid indirect String concatenation inside loops (more than 1 mn per api call)
                                        if not 'path' in associatedValueName.lower():
                                            json_findings = rest_service_aip.get_objectviolation_findings_json(objviol.componenthref, objviol.qrid)
                                        
                                        logutils.logdebug("json_findings=" + str(json_findings))
                                        if json_findings != None: 
                                            ...
                                            logutils.logdebug("          fin_name=" + str(fin_name))
                                            logutils.logdebug("          fin_type=" + str(fin_type))
                                            logutils.logdebug("          fin_values=" + str(fin_values))
                                            logutils.logdebug("          fin_bookmarks=" + str(fin_bookmarks))
                                            logutils.logdebug("          fin_parameters=" + str(fin_parameters))
                                    ...
                                    if displaysource:
                                                ...
                                                for bkm2 in bkm:
                                                    ...
                                                    try:
                                                        ...
                                                        if partialfiletxt == None:
                                                            logutils.loginfo("Second try without the line numbers")
                                                            partialfiletxt = rest_service_aip.get_sourcecode_file_json(filehref, None, None)
                                                            if partialfiletxt != None:
                                                                logutils.loginfo("Second try worked")
                                                        if partialfiletxt == None:
                                                            logutils.logwarning("Couldn't extract the file " + filehref)
                                                        if partialfiletxt != None:
                                                            filewithlinesnumber = ''
                                        ...
                                        elif objviol.sourcecodeshref != None:
                                            json_sourcescode = rest_service_aip.get_sourcecode_json(objviol.sourcecodeshref)

                                            logutils.logdebug("sourceCodesHref=" + objviol.sourcecodeshref)
                                            logutils.logdebug("json_sourcescode=" + str(json_sourcescode))
                                    ...
                                    objviol_output.appName = objapp.name
                                    objviol_output.snapshotdate = snapshot.isodate 
                                    objviol_output.snapshotversion = snapshot.version
                                    ...
                                    objviol_output.violationsStatus = objviol.violationstatus
                                    objviol_output.componentStatus = objviol.componentstatus
                                    ...
                                    outputline += ";" + str(objviol_output.violation.qrid)+ ";" + str(objviol_output.violation.qrname) + ";" +  str(objviol_output.violation.qrcritical) + ";" + str(objviol_output.maxWeight) + ";" + str(objviol_output.compoundedWeight) + ";" + str(objviol_output.compoundedWeightFormula) 
                                    ...
                                    outputline += ";" + ','.join(objviol_output.violation.modules)
                                    ...
                                    if generate_ac_model and detaillevel == 'Full' and objviol_output.violation_metrics.componentType != None and objviol.componentNameLocation != None:
                                        ...
                                        str_type = metamodel.get_metamodel_type(objviol_output.violation_metrics.componentType)
                                    ...
                                    strlistbc = ''
                                    #mapbctc
                                    b_robustness,b_efficiency,b_security,b_changeability,b_transferability = False,False,False,False,False
                                                ...
                                                for tcid in qrlisttc:
                                                    if tc == tcid:
                                                        ...
                                                        if bcid == "60011": 
                                                            ...
                                                            b_transferability = True
                                                        if bcid == "60012": 
                                                            strlistbc = strlistbc + '60012#Changeability,'
                                                            b_changeability = True
                                    if strlistbc != '': strlistbc = strlistbc[:-1]
                                    objviol_output.strlistbc = strlistbc
                                    outputline += ";" + objviol_output.strlistbc

                                    if b_robustness: outputline += ';TRUE' 
                                    else:outputline += ";" 
                                    if b_efficiency: outputline += ';TRUE' 
                                    else:outputline += ";"
                                    if b_security: outputline += ';TRUE' 
                                    else:outputline += ";"
                                    if b_changeability: outputline += ';TRUE' 
                                    else:outputline += ";"
                                    if b_transferability: outputline += ';TRUE' 
                                    else:outputline += ";"                                    
                                    ...
                                    if objviol.propagationriskindex != None: 
                                        objviol_output.pri_selected_bc = str(objviol.propagationriskindex)
                                    else: 
                                    ...
                                    if objviol.hasActionPlan:
                                        ...
                                        outputline += ";;;" 
                                    if objviol.hasExclusionRequest != None:
                                        exclusionRequest = objviol_output.violation.exclusioncomment
                                    ...
                                    #########################################################################
                                    currentviolurl = ''
                                    currentviolpartialurl= snapshot.href + '/business/60017/qualityInvestigation/0/60017/' + firsttechnicalcriterionid + '/' + objviol_output.violation.qrid + '/' + objviol_output.violation.componentid
                                    currentviolfullurl= rootedurl + '/engineering/index.html#' + snapshot.href + '/business/60017/qualityInvestigation/0/60017/' + firsttechnicalcriterionid + '/' + objviol_output.violation.qrid + '/' + objviol_output.violation.componentid
                                    currentviolurl = currentviolfullurl
                                    outputline += ";" + currentviolurl
                                    outputline += ";"+ str(qrrulepatternhref) + ";" + str(componentHref) + ";" +str(objviol.findingshref)         
                                    ...
                                    try:
                                        ...
                                        '''if len(srcCodeReference) > 0:
                                            for srcref in srcCodeReference:
                                                outputline += ';"'+ srcref + '"'                                    
                                        '''
                                        logutils.logdebug("  #code bookmarks="+str(len(srcCode)), False)
                                        if len(srcCode) > 0:
                                            iNbCodeCol = 0
                                            for src in srcCode:
                                                iNbCodeCol += 1
                                    ...
                                    if createexclusions and exclusionRequest == None:
                                        logutils.loginfo("Excluding the violation")
                                    ...
                                    if createactionplans:# and not objviol_output.violation.actionPlan:
                                        logutils.loginfo("Adding the violation to action plan")
                            ...
                            if generate_ac_model and detaillevel == 'Full':
                                logutils.loginfo("Creating file %s"  + acmodel.filename)
                            ...
                            if createexclusions and json_new_scheduledexclusions != None and len(json_new_scheduledexclusions) > 0:
                                logutils.loginfo("Creating "  + str(len(json_new_scheduledexclusions)) + " new exclusions : " + str(json_new_scheduledexclusions))
                                rest_service_aip.create_scheduledexclusions(domain.name, objapp.id,snapshot.snapshotid, json_new_scheduledexclusions)
                            else:
                                logutils.loginfo("No exclusion created")
                                
                            logutils.loginfo("json_new_actionplans=%s" % str(json_new_actionplans))
                            if createactionplans and json_new_actionplans != None:
                                str_json_action_plans = str(json_new_actionplans).replace("'", '"')
                                
                                logutils.loginfo("Creating "  + str(len(json_new_actionplans)) + " action plan items " + str_json_action_plans)
                                if  str_json_action_plans == '[]':
                                    logutils.loginfo("No action plan itea to create, items were already created")
                                else:
                                    rest_service_aip.create_actionplans_json(domain.name, objapp.id,snapshot.snapshotid, str_json_action_plans)
                                    logutils.loginfo("Action plan items created")
                            else:
                                logutils.loginfo("No action plan created")                                    
                            
                            # Summary in the log for this application snapshot
                            msg = "******************************\n"
                            msg += "Application name : " + objapp.name + "\n"
                            msg += "Snapshot : " + snapshot.href + "\n"
                            ...
                            if outputextension != None:
                                fpath = get_filepath(outputfolder, objapp.name, outputextension)
                                logutils.loginfo("Generating " + outputextension + " file " + fpath)
                                if outputextension == 'csv' or outputextension == 'xlsx':
                                    generate_output_file(outputextension, header, outputdata, fpath)
                    ...
                    if automatedactionplan_create:
                        logutils.loginfo("Generating automated action plan")
                        ...
                        try:
                            ...
                            logutils.logwarning( 'Not able to load file "quality standards priority for action plan.csv"', True)
                        ...
                        try:
                            ...
                            logutils.logwarning('Not able to load file "quality rules priority for action plan.csv"', True)                                
                        ...
                        for qs in list_priority_qs_input:
                            if i_violations_processed < automatedactionplan_maxnumber:
                                if qs in set_quality_standard_used_in_app:
                                    ...
                                    logutils.loginfo("Prioritizing quality standard %s " % str(qs), True)
                                    ...
                                    for qrid in list_priority_qr_input:
                                        if i_violations_processed < automatedactionplan_maxnumber and qrid in qs_qr_set:
                                            if dic_violations_by_qualityrule.get(qrid) == None:
                                                continue
                                            logutils.loginfo("  Prioritizing & processing quality rule %s (%s violations)" % (str(qrid),len(dic_violations_by_qualityrule[qrid])), True)
                                    ...
                                    for qrid in qs_qr_set:
                                        if i_violations_processed < automatedactionplan_maxnumber and not qrid in set_qr_already_processed:
                                            if dic_violations_by_qualityrule.get(qrid) == None:
                                                continue                                                    
                                            logutils.loginfo("  Processing quality rule %s (%s violations)" % (str(qrid),len(dic_violations_by_qualityrule[qrid])), True)
                                            i_violations_processed, list_violations_to_add_to_ap = add_violations_for_automated_ap(qrid, set_qr_already_processed, dic_violations_by_qualityrule, i_violations_processed, automatedactionplan_maxnumber, list_violations_to_add_to_ap)
                                            set_qr_already_processed.add(qrid)
                        ...
                        logutils.loginfo("json_str_automated_action_plan=%s" % json_str_automated_action_plan)
                        logutils.loginfo("Number of violations selected for automated action plan (including violations already in action plan) : %s" % (str(i_violations_processed)), True)
                        logutils.loginfo("Number of violations to be added to automated action plan : %s" % (str(i_adding)), True)
                        logutils.loginfo("Creating automated action plan items ")
                        if  json_str_automated_action_plan == '[]':
                            logutils.loginfo("No automated action plan item to create, items were already created")
                        else:
                            rest_service_aip.create_actionplans_json(domain.name, objapp.id,snapshot.snapshotid, json_str_automated_action_plan)
                            logutils.loginfo("Action plan automated items created")
